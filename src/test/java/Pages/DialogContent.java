...
public class DialogContent extends _Parent {
    ...
    //public List<WebElement>newNameList=nameList;
    ...
    @FindBy(xpath = "(//ms-text-field/input)[6]")
    public WebElement integrationCode;

    @FindBy(xpath = "(//ms-text-field//input)[7]")
    ...
    public void editAndDeleteFunction(String countryName, String editOrDelete)
    {
       ...
       {wait.until(ExpectedConditions.invisibilityOfAllElements(msjContainer));}}
        ...
        List<WebElement> btnList = new ArrayList<>();      // bu kısmı NEDEN GRİ????

        if (editOrDelete.equalsIgnoreCase("delete"))
           // btnList = waitVisibleListAllElement(deleteButtonList);
            btnList=deleteButtonList;

        else // delete dışıda ne gelirse,edıt butonlarının oluşmasını bekle.
...
//oluşan edit yada delete butonlarındaki, aranan ulkenın edıt yada deletesine tıkla.
...
//BU KISIM SADECE ARANAN ULKENIN EDIT VEYA DELETE BUTONUNA BASMAK İÇİNDİR.BAŞKA DEĞİL
    }
